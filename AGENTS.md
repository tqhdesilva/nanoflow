...
Loop: outer rollout phase samples T-step SDE trajectories under `theta_old`, recording per-step Gaussian log-probs and means. The inner update phase runs PPO-style `num_inner` epochs over the same rollout: recompute log-probs and means under `theta`, clipped IS surrogate, plus closed-form Gaussian KL to the frozen reference policy.
